...
| `src/tavern.js` | **the tavern** — the home room and where navigation is headed. Its own canvas scene (`tavern`, `TAV`): the hero's own sprite idles and walks the floor, the room's people stand at their posts, and the furniture is drawn procedurally here (`tavArt`) since a dungeon tileset has no table, bar or stool in it. **That furniture is the only art in the game not cut from the sheet, so it is the only art that can drift away from it**: `TAV_PAL` is sampled straight out of `assets/dungeon_tiles.png` and the sheet's idiom comes with it — a hard `#222222` outline round every object, a three-step ramp per material, one bright edge where the light lands, and a shallow top ellipse on anything round (the sheet's own cue — see its column cap). Never mix a new shade; the smoke test fails if a colour in `TAV_PAL` isn't a colour the sheet uses. Four **stations** — `Schmiede` → upgrade, `Bücherei` → study, `Gang` → combat, plus the `Schänke`; tapping a station's chip — or the picture itself — walks the mage over and hands the phase off **on arrival**, never before. The **bar is the one station that leads nowhere**: arriving opens an order instead (`openBarOrder`), a stage machine on `tavern.meal` — order → reply → serve → eat → cheer — where the missing word of an Italian line IS what the keeper puts on the counter (`TAV_MEAL`, seven servings drawn to the same rules as the furniture), and finishing it grants a shield worth `CONFIG.meal.shieldFraction` of max HP. **All of the exchange is drawn IN the scene** — speech in pixel bubbles over the two people talking, the menu as plaques below (`layoutBarMeal`, `bakeBubble`, `src/pixel-font.js`). It was a DOM panel across the foot of the screen first and that was the wrong object: the one thing in this game that is *happening in the room* was being narrated by an interface pasted over it. Two things follow — the text is **baked** into small canvases when the stage changes rather than drawn glyph by glyph each frame, and the menu is **hit-tested against those baked rects** in `tavernTapPoint`, since a plaque painted on a canvas is not a button the browser knows about (the smoke test presses the pixels for exactly that reason, and the station chips are `hushed` meanwhile so nothing floats over the tap targets). It TOPS UP rather than stacks, and it waits in `state.mealShield` until `startRun` pours it into `state.heroShield` — that pool is cleared at run start, so a meal written straight into it would be eaten by the walk into the hall. A wrong word costs nothing: the keeper shrugs, the word is struck off the menu and the order stands. Chips are DOM buttons placed over the canvas from art coordinates (`placeTavernChips`), so they go through the same delegated `data-act` dispatch as every other button and stay crisp. Layout is derived from the art size (x off the room's width, y as a fraction of the floor's depth), so the same room lays itself out on any phone |
