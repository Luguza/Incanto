...
| `src/skilltree.js` | the upgrade phase: an **authored** PoE-style rune tree (~800 nodes) plus purchase/reveal logic, the derived stat model (`recomputeMods`) and the pan/zoom SVG screen (`renderUpgradeFull`). **Where you add or move nodes:** the `ARMS` table. Twelve arms leave the seed, grouped around the circle by build identity (offence · Fortuna · sustain · defence) rather than alternating. Each runs `prelude` (**2–6** cheap nodes, per arm) → its **key** one ring past it (a spell's unlock node, or a generic arm's notable — always visible via `beacon`) → `branches` (2–5, per arm). **Every ring in that table is typed by hand and there is no PRNG**: a branch carries its own `fork` ring, its long/short twig tips (`short: 0` = never forks), and its `spur(at, len, tip)` dead ends. Branch content is a short list of `A.*` archetypes it cycles outward. Prices come from the archetype base × ring^0.9, snapped by `priceRound` onto a human ladder, then nudged per branch (`toll`) or per node (`tweak`); uniques are priced individually at their definition. Rings are bookkeeping (cost + value); positions come from `radialSlices` + `relaxTree` (angular slices by subtree size, then springs/repulsion for even spacing), so don't read geometry off the ring |
