...
| `src/skilltree.js` | the upgrade phase: an **authored** PoE-style rune tree (~1165 nodes) plus purchase/reveal logic, the derived stat model (`recomputeMods`) and the pan/zoom SVG screen (`renderUpgradeFull`). **Where you add or move nodes:** the `ARMS` table. Twelve arms leave the seed, alternating spell / generic; each runs `prelude` (rings 1–4, cheap generic nodes) → its **key** at ring 5 (a spell's unlock node, or a generic arm's notable — always visible via `beacon`) → `branches` (five aspect branches on a spell arm, three on a generic one) that fork in two at ring 13 and end in unique keystones at ring 20. Branch content is written as a short list of `A.*` archetypes it cycles outward |
