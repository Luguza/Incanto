...
| `src/grammar.js` | **the grammar curriculum**, data only: `GRAMMAR_UNITS` (ten units, the whole A1 road) and `GRAMMAR_LECTURES` (one topic each — `pages` that EXPLAIN it in German, then `CONFIG.grammar.drillCount` drills). A drill is authored as a COMPACT SPEC (`pick` / `write` / `pair` / `gap` / `build` / `para`), never as a literal question object: `buildDrill` in `lecture.js` writes the contract the renderers read — token arrays, blank indices, bank tile ids — once, instead of it being hand-written a few hundred times over. House rules are above the pool and enforced by `node tools/check-grammar.mjs`. Two of them decide how a lecture may be written. **A lecture uses only what the road behind it has taught**, plus its own — the audit walks the lectures in this array's order and grows the allowed set as it goes, so an INFLECTION is gated by the lecture that teaches it (a plural after "Einer oder viele?", agreement after "Adjektive auf -o", a verb's six forms after that verb's own lecture) rather than being free because its dictionary form is in `WORD_POOL`. What a lecture hands over it declares in `opens` (`noun-plural`, `adj-forms`, `verb:essere`, `verb:-are`, …), single forms in `teaches`. The first lecture in the game once asked the learner to build `Il libro è nuovo` — essere nineteen lectures early, agreement six — to test which article `libro` takes; that is why the early lectures drill NOUN PHRASES rather than sentences, which is what a chapter on gender and articles has always drilled. And a page **table types its columns** (`cols`, one of `label`/`it`/`de` each), because the renderer paints a cell for what it holds: the muted label face is for the column that names the ROW — the person in a paradigm — and was applied by POSITION until a table of `il figlio · la figlia` came out reading as if the masculine column were a caption for the feminine one. DISTRACTORS are exempt from the vocabulary check: a wrong option is supposed to be a form that does not exist |
...
| `src/quiz.js` | vocab-quiz logic + exercise handlers (`quizChoose`, `buildQuiz`, …); `quizReward` applies the banked multiplier plus the question's own stake, `advanceQuiz` cashes the multiplier in on the last question. Also the **conjugation ladder**: `makeConj` deals a rung of `CONFIG.conjugation.levels` (zuordnen: one verb's paradigm → zuordnen: forms from several verbs → pick a form → write a form → half a table → the whole paradigm), `noteConjResult` moves `state.conjLevel` up or down on the top rung. `settleQuiz` and `advanceQuiz` each fork once on `state.quizMode`: a lecture pays gems and ends on its scorecard, a quiz pays gold and cashes the banked multiplier in |
| `src/lecture.js` | the grammar half of the study phase: the Bücherei hub, the lecture shelf, the drill builder, the explain-page renderer, the gem payout and the lecture record. **A lecture runs on the QUIZ's own screen**, which is why this file is small — the quiz's body renderers turned out to be topic-agnostic (none of them has heard of `WORD_POOL`) and both learning-history hooks bow out of a question carrying no vocabulary, so a grammar drill is an ordinary question object and there is no second exercise engine. A lecture's pages are steps of the same `state.quizList` its drills are (`type: "explain"`), because a page that can be skipped is a page nobody reads. `state.quizMode` is the ONLY thing deciding whether a right answer pays gold or gems |
| `src/screens.js` | full-screen DOM renderers (innerHTML into `#app`). `renderQuizFull` is **one screen serving two things** — the vocab quiz and a grammar lecture — because the body renderers were already topic-agnostic; what forks is the header row (`quizHeaderMeta`), the `explain` case in the body switch, and which currency the feedback banner names. Also the combat HUD — which is **the hero's bar and nothing else**. There is no enemy health bar in this game: not under the scene (the wave line and the front-body bar that used to sit there are gone) and **not in the hall either** — a small per-body gauge painted onto the canvas was built and thrown out, because the scene is 200x80 art px, bodies queue a 16 px tile apart, and "127/176" is 27 px wide, so the readouts collided on every camp and sat over the sprites they described. What a body is worth is read off the body: its size, its sprite, and the damage numbers popping off it. Don't add one back. The strip is **one row** — bar across, figures at its right-hand end, no caption — because the rune circle under it is HEIGHT-limited on every phone, so a row spent here is diameter taken off the thing the thumb aims at. For the same reason `ARENA_VIEWBOX` crops the arena to the wheel's own extent (285 of the 300 it is authored in); that crop and the arena's bottom margin in `combat.css` are **one calculation** — the crop draws the wheel bigger, which walks it at the open book — so re-derive both together, by walking the pages' top edges in screen px (the closest approach is a tangent partway along the slope, not the vertical gap at the notch) |
| `src/tavern.js` | **the tavern** — the home room, and **the screen the game opens on**: `freshState` names it and `main.js` boots straight into it without starting a run, so the first thing the player sees is the room all three phases have a door in rather than a fight already under way. A run begins when the mage reaches the `Gang`, never before. Its own canvas scene (`tavern`, `TAV`): the hero's own sprite idles and walks the floor, the room's people stand at their posts, and the furniture is drawn procedurally here (`tavArt`) since a dungeon tileset has no table, bar or stool in it. **That furniture is the only art in the game not cut from the sheet, so it is the only art that can drift away from it**: `TAV_PAL` is sampled straight out of `assets/dungeon_tiles.png` and the sheet's idiom comes with it — a hard `#222222` outline round every object, a three-step ramp per material, and one bright edge where the light lands. Never mix a new shade; the smoke test fails if a colour in `TAV_PAL` isn't a colour the sheet uses. **The sheet also fixes the perspective, and it is not a matter of taste**: a box shows its front and its top, never a side, and the top is HALF as deep as the surface it caps — the sheet's crate is 16 px deep and shows 8 rows of lid, its closed chest 7, its column cap ~6 for 14. `TAV_TILT` is that half and `tavTop`/`tavRy` are the only ways to spend it, so no surface in the room can drift from the sheet. The top face is drawn full width (a tilt, not a vanishing point), lighter than the front, with exactly one bright line where the two planes meet and the `#222222` outline along the far edge — and the same half decides the floor, so a leg standing deeper in the room ends higher up the screen (`tavLeg`) and a thing set down on a table sits at the middle of its top face, not at the top of its bounding box (`tavArt`'s `top`, which is what the candles read). The same half runs round a CYLINDER too: a barrel's iron hoops are ellipses (`tavHoop`), level at the silhouette and dipping toward the camera in the middle — ruled flat they say the surface they lie on is flat, and two straight lines beat a curved lid and tapered staves put together. It also decides **where a shadow goes**: a footprint is an ellipse (`tavFoot` per piece, `tavShadow` to bake it) whose NEAREST point is the row the thing meets the boards, so the pool lies BEHIND the feet — centred on them, half of it sits in front of the contact point and the piece reads as hovering over its own shadow. It is flat-cored rather than linearly faded and thrown `TAV_SUN` px right, toward the one light in the room, because a pool that is neither is a pool the object's own silhouette covers entirely. **Heights are set against each other, not chosen**: table 11 px, its stools 8, counter 15, bar stools 10 (`tavStool(true)`, which is a different piece of furniture with a footrest, not the same stool moved) — measured off the 22 px mage, and a stool at half its counter is a footstool at a lectern. **Furniture OVERLAPS on purpose**: the depth sort turns overlap into a room, so a stool authored behind a table comes back with its seat under the slab (`tavSeating`) and a layout number reads as "which of these is in front", not "how far apart". Four **stations** — `Schmiede` → upgrade, `Bücherei` → study, `Gang` → combat, plus the flavour `Schänke`; tapping a station's chip — or the picture itself — walks the mage over and hands the phase off **on arrival**, never before. Chips are DOM buttons placed over the canvas from art coordinates (`placeTavernChips`), so they go through the same delegated `data-act` dispatch as every other button and stay crisp. Layout is derived from the art size (x off the room's width, y as a fraction of the floor's depth), so the same room lays itself out on any phone. Walking is **A\* over a 4 px grid of the floor** (`buildTavernNav` / `tavRoute` / `tavWalkTo`, on the same vendored library the corridor's lane routing uses), built once with the room since the furniture never moves and kept as CORNERS so the walk is still long strides. It replaced a slide-along-the-blocked-axis step that simply ENDED the walk when both axes were blocked at once — which made every layout number load-bearing: the smoke test reached the bar because one diagonal cleared a barrel by two px, so moving that barrel broke the room invisibly. Crowding the furniture is only free because the search is real |
| `src/nav.js` | bottom phase-switcher nav (`navTo`, pixel-art icons) — renders into `<nav id="bottom-nav">`. Four buttons: **tavern first**, then study · forge · combat — first in the bar because it is also where the game starts, so the bar reads left-to-right as the room and the three doors out of it. `navTo("combat")` is what STARTS a run when none is live, which is why booting into the tavern needs nothing else: the hall waits for the tap. The study button lands on the **Bücherei hub** (vocab quiz · grammar), not straight in a quiz — but only when there is nothing to resume: `studyResumeOr` is the one rule, and the three doors into the phase all follow it, so an unfinished session (quiz or lecture, they share the fields) is never wiped by hopping away |
...
`dmgMult`, `hpMult` or `attackMs`, the windup, hero HP, armour or regen — run the attrition tool,
...
survives one plain skeleton's attention, the hero's own plate included — under
~25 s and a single camp settles the run instead of grinding it down. (Seconds
rather than blows: a blow is no longer a fixed bite now that every body swings on
its own cadence.) The **chapter table** is the grind itself: LP lost
...
comparison between two sets of numbers, not as a prediction — and the early
chapters reading 0 for a finished build is expected, not a finding.

The committed balance, for comparison: 3 camps cleared on a bare hero, 5 at 2 %
of `treeGold`, 28 at 10 %, 61 at 30 %, 78 at 60 %, and out of the door at 90 %.
**Both tools have to agree**, because the two sides move together — a change to
`treeTotals` shows up in `stat-supply` as power and in `attrition` as depth, and
raising one side to answer the other is how a rebalance ends up changing nothing.

When you touch `SENTENCE_POOL` or `GRAMMAR_LECTURES`, also run the content
audits — those are the two places hand-written Italian lives, so a typo or a
word outside `WORD_POOL` has nothing else to catch it:
...
node tools/check-grammar.mjs     # lecture structure, block shapes, drill specs,
                                 # typed table columns, and that no lecture uses
                                 # grammar a later lecture is what teaches
```

Both answer "is this a word the game teaches?" out of one shared list
(`tools/lib/italian-vocab.mjs`) — two lists that could disagree would let a word
through one door and stop it at the other, and the door that let it through is
the one nobody would notice. A lecture is a worse place for a typo than a
sentence is: a bad sentence is one question in a random draw, a bad lecture is a
fixed page somebody is being *taught* from. The smoke test therefore builds and
renders **every drill of every lecture** and checks the right answer is actually
reachable on the built page — a compact spec the builder can't expand would
otherwise ship as a blank stage in a lecture nobody opens for a month.
