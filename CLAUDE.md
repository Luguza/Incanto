...
| `src/encounters.js` | **where the hall is designed**: `SHAPES` (bare formations), `PACKS` (a shape filled with variants), `CHAPTERS` (16 of them — a 3-camp slime prologue that is not a fight, then 15 that each introduce one new body alone before using it in force), the derived `ENCOUNTER_PLAN` (163 camps on a fixed 2.5 m cadence) and `HALL_END_METRES` — the corridor is FINITE and ends at a door. Deterministic — no randomness. `previewPlan()` / `previewBestiary()` dump it to the console |
