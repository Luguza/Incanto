...
| `src/config.js` | `CONFIG` — all gameplay numbers, flags, colours, and **`treeTotals`: the balance table** — how much of each stat the whole skill tree contains. No runtime caps, soft caps or diminishing returns exist anywhere; a stat's total is its ceiling because it bounds the supply, and every node pays exactly what it prints. Damage is built in three stages (Kern → Verstärkung → Zuschlag) |
