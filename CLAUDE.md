...
| `src/config.js` | `CONFIG` — all gameplay numbers, flags, colours. Damage is built in three stages (Kern → Verstärkung → Zuschlag, see `caps`): the two FLAT pools are uncapped so a node's printed number always arrives whole, only the percentages soft-cap |
...
| `src/skilltree.js` | the upgrade phase: an **authored** PoE-style rune tree (~1050 nodes) plus purchase/reveal logic, the derived stat model (`recomputeMods`) and the pan/zoom SVG screen (`renderUpgradeFull`). **Where you add or move nodes:** the `ARMS` table. The damage archetypes come in three kinds matching the three stages — `dmgBaseFlat`/`dmgBasePct` feed the Kern, `dmgPct` multiplies, `dmgFlat` ("Schneide") lands last of all inside `spellPower` and is never capped. Twelve arms leave the seed, alternating spell / generic; each runs `prelude` (rings 1–4, cheap generic nodes) → its **key** at ring 5 (a spell's unlock node, or a generic arm's notable — always visible via `beacon`) → `branches` (five aspect branches on a spell arm, three on a generic one) that fork in two around ring 13 and end in unique keystones out near ring 19, with dead-end offshoots hanging off them. Branch content is written as a short list of `A.*` archetypes it cycles outward. Rings are bookkeeping (cost + value); positions come from `radialSlices` + `relaxTree` (angular slices by subtree size, then springs/repulsion for even spacing), so don't read geometry off the ring |
