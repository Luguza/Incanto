...
| `src/config.js` | `CONFIG` — all gameplay numbers, flags, colours. **No balance ceilings live here any more:** there are no soft caps, no hard caps and no diminishing returns anywhere in the game. Damage is built in three stages (Kern → Verstärkung → Zuschlag), and every stat arrives exactly as the nodes granted it |
